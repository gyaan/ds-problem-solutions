/* Problem : Remove duplicate char from a given string
...
string removeDuplicateUsingSoringMethod(string);
...
int main() {
   ...
   //using brute force method
	...
	//first  sort the string and then remove duplicate
	/*lets say we used any sorting technique and
	sort the string and pass it to below function */
    string2 = removeDuplicateUsingSoringMethod("aadddeeef");
...
string removeDuplicateUsingSoringMethod(string str) {
...
string removeDuplicateUsingHashMapMethod(string str) {

	map<char, int> ht; //define hash map

	int stringLength = str.length(); //get the length of string

	for (int i = 0; i < stringLength - 1; i++) {
		if (ht.find(str[i]) == ht.end()) {
			//if string char is not in hash then store it to hash map
			ht[str[i]] = 1;
		}
	}

	map<char, int>::iterator dis;  //create hash map iterator
	string temp = "";  //temp string
	for (dis = ht.begin(); dis != ht.end(); dis++) {
